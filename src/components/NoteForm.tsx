...
export function NoteForm({ 
        ...
        try {
          ...
          if (metadata.success && metadata.title) {
            setFormData(prev => ({
              ...
              category: isYouTubeUrl ? youTubeCategory : prev.category
            }));
  ...
  }, [formData.url, fetchMetadata, fetchWebMetadata, lastProcessedUrl, isYouTubeUrl, youtubeSettings.autoAddChannelAsTag, youTubeCategory]);
  ...
  const addChannelNameToTags = (channel: string, currentTags: string | string[]) => {
    ...
    // Ensure Note tag is present
    if (!tagList.includes('Note')) {
      tagList.unshift('Note');
    }
    ...
    <form onSubmit={handleSubmit} className="bg-[#1a1a1a]/50 border border-white/10 rounded-xl p-4">
      ...
      <div className="mb-4">
        ...
        <p className="mt-1 text-xs text-white/40">
          Tags (optional, comma-separated). "Note" tag will be automatically added.
